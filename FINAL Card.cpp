...
bool Card_less(const Card &a, const Card &b, Suit trump){
  ...
  if(a_rbower){return false;} // a is right bower beats all
  if(b_rbower){return true;}  // b is right bower beats all
  
  if(a_lbower && !b_rbower){return false;} // a is left bower beats all except right
  if(b_lbower && !a_rbower){return true;}  // b is left bower beats all except right
  
  if(a_trump && b_trump){return a < b;} // both are trump compare rank
  if(a_trump && !b_trump && !(b_lbower)){return false;} // a is trump b is not
  if(!a_trump && b_trump && !(a_lbower)){return true;}  // b is trump a is not
  
  if(!(a_trump) && !(b_trump)) {return a < b;} // both not trump a lower rank
  
  return a < b; // fallback compare (shouldn’t hit)
}
...
bool Card_less(const Card &a, const Card &b, const Card &led_card, Suit trump){
  ...
  return a < b; // fallback suit compare
}
